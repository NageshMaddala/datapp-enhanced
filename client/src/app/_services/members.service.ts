...
export class MembersService {
  ...
  getMembers(userParams: UserParams) {
    ...
    // HttpParams is provided by angular

    // add each filter with a key and store the results in memory
    console.log(Object.values(userParams).join('-'));

    const response = this.memberCache.get(Object.values(userParams).join('-'));

    // If the filter cache contains data
    // then return straight from the cache using the of Observable
